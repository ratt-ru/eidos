...
def inspect_dict(dictionary, nu, idxs=None, show=True, Noll=False, ylim=[None,None], xlim=None, marker='-',\
    labels=None, colors=None, lw=2, msize=2, figsize=(20,10), share=False):
    plt.rcParams['figure.figsize'] = figsize
    plt.rcParams['font.size'] = 12
    if share==True: f, axes = plt.subplots(2,2, sharex=True, sharey=True)
    else: f, axes = plt.subplots(2,2, sharex=True, sharey=False)
    ...
    for i in range(2):
        for j in range(2):
            corr = corrs[c]
            mod, modpol, dat = dictionary[corr], dictionary['polar'][corr], dictionary['data'][corr]
            ampl, sinp, cosp = modpol[0,...], modpol[1,...], modpol[2,...]
            idxs = dictionary[corr]['zind']
            
            if Noll==False:
                avg = np.mean(ampl, axis=1)
                sortidx = np.argsort(avg)[::-1]
            ax = axes[i,j]
